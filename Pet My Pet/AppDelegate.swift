...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        //To set the default view controller when login token is present
        let mainStoryBoard = UIStoryboard(name: "Main", bundle : nil)
        self.window = UIWindow(frame: UIScreen.main.bounds)
        if let _: String = KeychainWrapper.standard.string(forKey: "savedToken") {
            let mainViewController: MainViewController = mainStoryBoard.instantiateViewController(withIdentifier: "mainvc") as! MainViewController
        ...
        //To set the color on navigation bar as well as its text and status bar icon colors
        ...
        statusBarView?.backgroundColor = UIColor(red: 33/255, green: 33/255, blue: 33/255, alpha: 1)
        return true
    }
